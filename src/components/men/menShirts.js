...
export class MenShirts extends Component {
  ...
  componentDidMount() {
    //Destructor shirts from the context curretly used
    // console.log(this.context);
    let { menShirts } = this.context;
    this.componentLoaded = true;

    //Using setState to set the change the values
    this.setState(
      {
        //Assigning clothes to shirts
        clothes: [...menShirts]
      },
      () => {
        this.setState(
          {
            //Assigning filteredClothes to shirts
            filteredClothes: [...menShirts]
          },
        ...
        if (sizes.length > 0) {
          ...
          this.setState(
            (prevState, props) => ({
              ...
              filteredClothes: prevState.filteredClothes.filter(shirt => {
                //Creates a variable called hasSize ans sets it to 0
                let hasSize = 0;
                //Loops through all the sizes in each shirt
                ...
                //If hasSize is greater than or equal to one, that means that the shirt has at least
          ...
          this.setState(
            (prevState, props) => ({
              ...
              filteredClothes: prevState.filteredClothes.filter(shirt => {
                //Creates a variable called hasSize ans sets it to 0
                let hasColor = 0;
                //Loops through all the images in each shirt because each image has a images source
          ...
          this.setState(
            (prevState, props) => ({
              ...
              filteredClothes: prevState.filteredClothes.filter(shirt => {
                ...
                if (shirt.sale !== null)
                  return parseFloat(shirt.sale) >= parseFloat(lowPrice);
                //If it does not has a sale price, the regular price is compared to the lowPrice entered
                else return parseFloat(shirt.price) >= parseFloat(lowPrice);
              })
            }),
            //When the setState is completed, call the sortClothes functions which will sort
            //the array according to the currently selected sort
          ...
          this.setState(
            (prevState, props) => ({
              ...
              filteredClothes: prevState.filteredClothes.filter(shirt => {
                ...
                if (shirt.sale !== null)
                  return parseFloat(shirt.sale) <= parseFloat(highPrice);
                //If it does not has a sale price, the regular price is compared to the lowPrice entered
                else return parseFloat(shirt.price) <= parseFloat(highPrice);
              })
            }),
            //When the setState is completed, call the sortClothes functions which will sort
            //the array according to the currently selected sort
          ...
          <div className="row w-100">
            <div className="col-6 d-none d-lg-block">
              <div className="row align-items-center">
                <h1 className="d-none d-lg-block col-12">Men's Shirts</h1>
              </div>
            </div>
            <div className="col-12 mb-5 mt-5 mt-lg-0">
              <MenSortBy filterClothes={filterClothes} />
            </div>
            <p className="d-block col-12 col-12 text-right">
              <span className="lead mr-1">
                {this.state.filteredClothes.length}
              </span>
              Items in Men's Shirts
            </p>
            <div className="col-lg-3 col-md-12 d-lg-block">
              <MenFilterShirts filterClothes={filterClothes} />
            </div>
            {this.state.currentClothes.length ? (
              <div className="col-md-12 col-lg-9 mt-3 mt-lg-0">
                <div className="row">
                  {this.state.currentClothes.map(shirt => {
                    return <ItemPreview item={shirt} key={shirt.id} />;
                  })}
                </div>
              </div>
            ) : (
              <div>There are no shirts available</div>
            )}
          </div>
          <Pagination
